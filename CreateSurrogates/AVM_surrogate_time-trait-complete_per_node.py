...
for run in range(1,11):
    ...
    avm.op_nodes["op_synthetic"] = avm.op_nodes.groupby("node_id").op_synthetic.apply(shuffle)\
                     .reset_index().set_index("node_id").op_synthetic
    avm.op_nodes.reset_index(inplace=True)
    avm.op_nodes.to_pickle(f"{path}/op_nodes_shuffled_time_complete_per_node_phi{phi}_{run}.pkl")  

    exposure = avm_glasses.calc_exposure("expo_frac", "op_synthetic", exposure_time = 7)
    exposure.to_pickle(f"{path}/exposure_fitness_7_1_shuffled_time_traits_complete_per_node_phi{phi}_{run}.pkl")