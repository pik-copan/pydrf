...
for run in range(1,11):
    avm.op_nodes.set_index("time",inplace=True)
    avm.op_nodes.sort_index(inplace=True)
    avm.op_nodes["op_synthetic"] = avm.op_nodes.groupby("time").op_synthetic.apply(shuffle)\
                     .reset_index().set_index("time").op_synthetic
    avm.op_nodes.reset_index(inplace=True)
    avm.op_nodes.to_pickle(path + f"/op_nodes_fitness_shuffled_surrogate_trait_phi{phi}_{run}.pkl")
    
    exposure = avm_glasses.calc_exposure("expo_frac", "op_synthetic", exposure_time = 7)
    exposure.to_pickle(path + f"/exposure_fitness_7_1_shuffled_surrogate_trait_phi{phi}_{run}.pkl")
