...
for run in range(5,30):
    ...
    #shuffle time
    cns.a_ij.edge = shuffle(cns.a_ij.edge.values)
    cns.a_ij.to_pickle(f"{path}/a_ij_shuffled_time_complete_{run}.pkl")
    
    exposure = cns_glasses.calc_exposure("expo_frac", "op_fitness", exposure_time = 7)
    exposure.to_pickle(f"{path}/exposure_fitness_7_1_shuffled_time_edges_complete_{run}.pkl")