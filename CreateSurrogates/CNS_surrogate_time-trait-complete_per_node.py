...
for run in range(15,30):
    ...
    cns.op_nodes["op_fitness"] = cns.op_nodes.groupby("node_id").op_fitness.apply(shuffle)\
                     .reset_index().set_index("node_id").op_fitness
    cns.op_nodes.reset_index(inplace=True)
    cns.op_nodes.to_pickle(f"{path}/op_nodes_shuffled_time_complete_per_node_{run}.pkl")  

    exposure = cns_glasses.calc_exposure("expo_frac", "op_fitness", exposure_time = 7)
    exposure.to_pickle(f"{path}/exposure_fitness_7_1_shuffled_time_traits_complete_per_node_{run}.pkl")